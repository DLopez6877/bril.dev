...
const Nav = () => {
  ...
  return (
    <div className="nav-container">
      <m.div className="full-nav" initial={{y: 0}} animate={{y: isScrolled ? "-100%" : 0}} transition={{type: "spring", stiffness: 200, damping: 20, delay: isScrolled ? 0 : 0.2}}>
        <Link to={logoTo} className="logo">
          <img height={30} src={logo} alt="Bril Logo." />
        </Link>
        <div className="links">
      ...
      <m.button
        ...
        transition={{type: "spring", stiffness: 150, damping: isScrolled ? 5 : 30, delay: isScrolled ? 0.6 : 0}}
        dragConstraints={{
          top: 0,
      ...
      <AnimatePresence>
        {isDrawerOpen && (
          <>
            ...
            <m.div className="overlay" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} transition={{type: "spring", stiffness: 120, damping: 20}} />
            <m.button onClick={closeDrawer} className={`close-button ${drawerSide === "left" ? "drawer-left" : ""}`} aria-label="Close Drawer" type="button" custom={drawerSide} variants={NavButtonVariants} initial="hidden" animate="visible" exit="exit">
              <div className="x">
