...
import bg from '../../assets/herobg-tall.png';
import coder from '../../assets/coder-tall.png';
...
const ParallaxHero = () => {
    ...
    const kotlinCode = 'class CardFactory { fun createCard(type: String): Card = when(type) { "creature" -> CreatureCard("Goblin", 1, 1); "spell" -> SpellCard("Fireball", "Deal 3 damage"); else -> LandCard("Mountain") } }                                                                                                                                       ';
    ...
    return (
        <div className="parallax-hero-container" onMouseMove={handleMouseMove}>
            <img
            ...
            {/* css */}
            <div className="parallax code css" style={{ transform: `translate(${xValue / 2.5}px, ${yValue / 2.5}px)` }}>
                <InfiniteScrollText scrollSpeed={0.05} fontSize='3.07rem'>{cssCode}</InfiniteScrollText>
            </div>

            {/* js */}
            <div className="parallax code js" style={{ transform: `translate(${xValue / 1.5}px, ${yValue / 1.5}px)` }}>
                <InfiniteScrollText scrollSpeed={0.03} fontSize='2.92rem' direction='right' reverseOnScroll='false'>{jsCode}</InfiniteScrollText>
            </div>

            {/* html */}
            <div className="parallax code html" style={{ transform: `translate(${xValue / 1.5}px, ${yValue / 1.5}px)` }}>
                <InfiniteScrollText scrollSpeed={0.01} fontSize='2.85rem' direction='left' reverseOnScroll='false'>{htmlCode}</InfiniteScrollText>
            </div>

            {/* python */}
            <div className="parallax code python" style={{ transform: `translate(${xValue / 2}px, ${yValue / 2}px)` }}>
                <InfiniteScrollText scrollSpeed={0.05} fontSize='2.62rem'>{pythonCode}</InfiniteScrollText>
            </div>

            {/* csharp */}
            <div className="parallax code csharp" style={{ transform: `translate(${xValue / 3.5}px, ${yValue / 3.5}px)` }}>
                <InfiniteScrollText scrollSpeed={0.0} fontSize='2.39rem'>{csharpCode}</InfiniteScrollText>
            </div>

            {/* sql */}
            <div className="parallax code sql" style={{ transform: `translate(${xValue / 3}px, ${yValue / 3}px)` }}>
                <InfiniteScrollText scrollSpeed={0.01} fontSize='2.16rem'>{sqlCode}</InfiniteScrollText>
            </div>

            {/* java */}
            <div className="parallax code java" style={{ transform: `translate(${xValue / 4}px, ${yValue / 4}px)` }}>
                <InfiniteScrollText scrollSpeed={0.0} fontSize='1.94rem'>{javaCode}</InfiniteScrollText>
            </div>

            {/* test */}
            <div className="parallax code test" style={{ transform: `translate(${xValue / 4}px, ${yValue / 4}px)` }}>
                <InfiniteScrollText scrollSpeed={0.0} fontSize='1.71rem'>{testCode}</InfiniteScrollText>
            </div>

            {/* powershell */}
            <div className="parallax code powershell" style={{ transform: `translate(${xValue / 4}px, ${yValue / 4}px)` }}>
                <InfiniteScrollText scrollSpeed={0.0} fontSize='1.48rem'>{powershellCode}</InfiniteScrollText>
            </div>

            {/* go */}
            <div className="parallax code go" style={{ transform: `translate(${xValue / 4}px, ${yValue / 4}px)` }}>
                <InfiniteScrollText scrollSpeed={0.0} fontSize='1.25rem'>{goCode}</InfiniteScrollText>
            </div>

            {/* ruby */}
            <div className="parallax code ruby" style={{ transform: `translate(${xValue / 4}px, ${yValue / 4}px)` }}>
                <InfiniteScrollText scrollSpeed={0.19} direction='left' fontSize='1.03rem'>{rubyCode}</InfiniteScrollText>
            </div>

            {/* kotlin */}
            <div className="parallax code kotlin" style={{ transform: `translate(${xValue / 4}px, ${yValue / 4}px)` }}>
                <InfiniteScrollText scrollSpeed={0.0} fontSize='0.80rem'>{kotlinCode}</InfiniteScrollText>
            </div>
            ...
            <h1 className="text parallax">Bril Lopez</h1>
            <h2 className="text parallax">Frontend</h2>
            <h2 className="text parallax">Frontend Developer</h2>
            <img
                className='parallax coder'
                src={coder}
                alt="A headshot of a programmer wearing a hoodie."
                style={{ transform: `translate(${xValue}px, ${yValue / 6}px)` }}
            />
            <img
