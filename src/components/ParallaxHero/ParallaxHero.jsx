...
const ParallaxHero = () => {
    ...
    const csharpCode = 'using System; class Program { static void Main() { int num1 = 5, num2 = 10; Console.WriteLine("Sum: " + (num1 + num2)); } } using System; class Program { static void Main() { int num1 = 5, num2 = 10; Console.WriteLine("Sum: " + (num1 + num2)); } }';
            ...
            <>{/* #region __________ CODE __________ */}
                <div className="code-text">
                    ...
                    <div className="parallax code css" style={{ transform: `translate(${xValue / 0.5}px, ${yValue / 0.5}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} direction='right'>{cssCode}</InfiniteScrollText>
                    </div>

                    {/* js */}
                    <div className="parallax code js" style={{ transform: `translate(${xValue / 0.6}px, ${yValue / 0.6}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} direction='right'>{jsCode}</InfiniteScrollText>
                    </div>

                    {/* html */}
                    <div className="parallax code html" style={{ transform: `translate(${xValue / 0.7}px, ${yValue / 0.7}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} >{htmlCode}</InfiniteScrollText>
                    </div>

                    {/* python */}
                    <div className="parallax code python" style={{ transform: `translate(${xValue / 0.8}px, ${yValue / 0.8}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} >{pythonCode}</InfiniteScrollText>
                    </div>
                    ...
                    <div className="parallax code test" style={{ transform: `translate(${xValue / 1.3}px, ${yValue / 1.3}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} direction='right' >{testCode}</InfiniteScrollText>
                    </div>
                    ...
                    <div className="parallax code go" style={{ transform: `translate(${xValue / 1.8}px, ${yValue / 1.8}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} direction='right' >{goCode}</InfiniteScrollText>
                    </div>

                    {/* ruby */}
                    <div className="parallax code ruby" style={{ transform: `translate(${xValue / 2}px, ${yValue / 2}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} >{rubyCode}</InfiniteScrollText>
                    </div>

                    {/* kotlin */}
                    <div className="parallax code kotlin" style={{ transform: `translate(${xValue / 3}px, ${yValue / 3}px)` }}>
                        <InfiniteScrollText scrollSpeed={0.08} direction='right' >{kotlinCode}</InfiniteScrollText>
                    </div>
